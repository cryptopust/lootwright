...
await page.getByRole('button', { name: 'Devam', exact: true }).click();
const pobField = page.getByLabel(/PoB kodu veya pasted pobb.in/);
await pobField.fill(pob);
await page.getByRole('button', { name: 'Devam', exact: true }).click();
...
await page.getByLabel(/İşleme ve süreli saklama/).check();
await page.getByRole('button', { name: 'Devam', exact: true }).click();
const submitResponse = page.waitForResponse((r) =>
    r.url().includes('/api/analyses/wizard'),
);
await page.getByRole('button', { name: /Analizi|Gönder|Başlat/i }).click();
const sr = await submitResponse;
const body = await sr.json();
console.log('submit', sr.status(), body);
const id = body.analysis_id;
let result = null;

for (let i = 0; i < 20; i++) {
    await page.waitForTimeout(1500);
    const rr = await page.request.get(base + '/api/analyses/' + id);
    result = { status: rr.status(), body: await rr.json() };
    console.log(
        'poll',
        i,
        result.status,
        result.body?.analysis?.state ?? result.body?.state,
    );

    if ((result.body?.analysis?.state ?? result.body?.state) === 'completed') {
        break;
    }
}
