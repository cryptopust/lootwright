...
await page.getByRole('button', { name: 'Devam', exact: true }).click();
for (let index = 0; index < 4; index += 1) await page.getByRole('button', { name: 'Devam', exact: true }).click();
await page.locator('input[type=checkbox]').last().check();
await page.getByRole('button', { name: 'Devam', exact: true }).click();
const pending = page.waitForResponse((response) => response.url().includes('/api/analyses/wizard'));
await page.getByRole('button', { name: /Analizi|G.nd(er|er)|Ba.lat/i }).click();
const response = await pending;
const submission = await response.json();
console.log(JSON.stringify({ event: 'IMPORT_ACCEPTED', http: response.status(), analysis_id: submission.analysis_id, state: submission.status }));

const seen = new Set();
let body;
for (let index = 0; index < 90; index += 1) {
    await page.waitForTimeout(1000);
    const polled = await page.request.get(`${base}/api/analyses/${submission.analysis_id}`);
