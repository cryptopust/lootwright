...
final readonly class PoeNinjaSyncService
{
    ...
    {
        if (! (bool) config('external-sources.poe_ninja.enabled')) { return new SourceSyncResult(false, 0, 'source_disabled'); }
        if (! $this->policy->permits('poe_ninja.economy.leagues.fetch')) { return new SourceSyncResult(false, 0, 'policy_denied'); }
        $lock = Cache::lock('external-source:poe-ninja-sync', 900);
        if (! $lock->get()) { return new SourceSyncResult(false, 0, 'overlapping_sync'); }
        try {
            $leaguesResponse = $this->client->getLeagues();
            if ($leaguesResponse['status'] === 304) { return new SourceSyncResult(false, 0, 'league_snapshot_missing'); }
            $leagues = $this->normalizer->leagues($leaguesResponse['body']);
            $league = $requestedLeague ?? ($leagues[0]->name ?? null);
            if (! is_string($league) || ! in_array($league, array_map(static fn ($item): string => $item->name, $leagues), true)) { return new SourceSyncResult(false, 0, 'invalid_league'); }
            $count = 0;
            foreach ([...EconomyCategory::exchange(), ...EconomyCategory::stash()] as $category) {
                ...
                if (! $this->policy->permits($operation)) { return new SourceSyncResult(false, $count, 'policy_denied'); }
                $count += $this->syncCategory($league, $category, $operation);
            }
            return new SourceSyncResult(true, $count);
        } catch (PoeNinjaFailure $failure) { return new SourceSyncResult(false, 0, $failure->failureCode); }
    ...
    private function syncCategory(string $league, EconomyCategory $category, string $operation): int
    {
        ...
        $started = CarbonImmutable::now('UTC'); $response = $this->client->getOverview($league, $category, $previous?->etag, $previous?->last_modified);
        if ($response['status'] === 304) { return 0; } // preserves current valid snapshot
    ...
    {
        if (preg_match('/max-age=(\d+)/i', $headers['cache_control'], $matches) === 1) { return $fetchedAt->addSeconds(max(300, min((int) $matches[1], (int) config('external-sources.poe_ninja.refresh_seconds')))); }
