...
final class PoeNinjaNormalizer
{
    ...
    public function leagues(string $body): array
    {
        ...
        if (! is_array($rows) || ! array_is_list($rows)) { throw new PoeNinjaFailure('unexpected_leagues_schema', false); }
        $result = [];
        foreach ($rows as $row) {
            if (! is_array($row) || ! is_string($row['name'] ?? null) || trim($row['name']) === '') { throw new PoeNinjaFailure('unexpected_leagues_schema', false); }
    ...
    public function quotes(string $body, string $league, EconomyCategory $category, CarbonImmutable $fetchedAt, CarbonImmutable $expiresAt): array
    {
        ...
        if (! is_array($lines) || ! array_is_list($lines)) { throw new PoeNinjaFailure('unexpected_overview_schema', false); }
        $checksum = hash('sha256', $body); $quotes = [];
        foreach ($lines as $line) {
            if (! is_array($line)) { throw new PoeNinjaFailure('unexpected_overview_schema', false); }
            ...
            if (! is_string($name) || trim($name) === '' || ! is_string($externalId) || trim($externalId) === '' || (! is_int($value) && ! is_float($value))) { throw new PoeNinjaFailure('unexpected_overview_schema', false); }
    ...
    {
        try { $decoded = json_decode($body, true, 512, JSON_THROW_ON_ERROR); } catch (\JsonException) { throw new PoeNinjaFailure('malformed_json', false); }
        if (! is_array($decoded)) { throw new PoeNinjaFailure('unexpected_schema', false); }
        return $decoded;
    }
}
