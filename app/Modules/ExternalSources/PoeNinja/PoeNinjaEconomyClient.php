    ...
    private function get(string $operation, string $url, ?string $etag, ?string $lastModified): array
    {
        ...
        for ($attempt = 0; $attempt < 3; $attempt++) {
            try {
                ...
                if ($attempt === 2) {
                    throw new PoeNinjaFailure('connection_or_timeout', true);
                }
                usleep(random_int(50_000, 150_000) * ($attempt + 1));

                continue;
            }

            if ($response->redirect()) {
                throw new PoeNinjaFailure('redirect_denied', false);
            }
            if (($response->status() === 429 || $response->serverError()) && $attempt < 2) {
                $retryAfter = max(0, min(5, (int) $response->header('Retry-After')));
                usleep(($retryAfter > 0 ? $retryAfter * 1_000_000 : random_int(50_000, 150_000) * ($attempt + 1)));
            ...
            if (strlen($body) > self::MAX_RESPONSE_BYTES) {
                throw new PoeNinjaFailure('response_too_large', false);
            }
            if (! $response->successful() && $response->status() !== 304) {
                throw new PoeNinjaFailure('http_'.$response->status(), $response->serverError());
            }
