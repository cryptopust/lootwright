...
final class SubmitWizardAnalysisController extends Controller
{
    public function __invoke(SubmitWizardAnalysisRequest $request, SubmitBuildArtifact $useCase): JsonResponse
    {
        ...
        $artifact = $type === 'pob' ? $data['pob'] : ($type === 'item_text' ? $data['item_text'] : json_encode(['flow' => $data['flow'], 'class' => $data['character_class'], 'ascendancy' => $data['ascendancy'] ?? null, 'level' => $data['character_level']], JSON_THROW_ON_ERROR));
        ...
        $receipt = $useCase->handle(new SubmitBuildArtifactCommand((string) $request->user()->id, $data['idempotency_key'], GameEdition::Poe1, Locale::from('tr-TR')->value(), $type, $artifact, new AnalysisParameters($goals, $data['budget_amount'] ?? null, $data['budget_currency'] ?? null, new AnalysisSelection(PlatformRealm::Pc, $data['league'] ?? null, $data['goals'][0], null, null, null, (bool) $data['ai_explanation_opt_in'], $data['character_class'], $data['ascendancy'] ?? null, (int) $data['character_level'], $data['flow']))));
