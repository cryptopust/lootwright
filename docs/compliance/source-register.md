...
| `GGG-POE1-SKILLTREE-001` | Exact documented official PoE1 passive skill-tree export family; [GGG API Reference](https://www.pathofexile.com/developer/docs/reference) | GGG official source; exact revision, URL, retrieval time and checksum required per snapshot | Out-of-band operator import into an immutable PoE1 snapshot; may support a reviewed published ruleset | No protected media; attribution/non-affiliation notice retained | `allowed` only for the exact documented export and Policy Gate conditions |
