...
| `USER-INPUT-002` | PoB/PoB2 share code explicitly pasted or uploaded as plain text by the user | Directly supplied by the user; underlying format provenance is separate | Decode only after the applicable parser-format record is approved | Never publish or persist raw input; transient processing by default | `conditional`; input accepted only when parser record is active |
