...
| Deployment/proxy mistake | Mutable image, spoofed forwarding header, plaintext data-store link, public Horizon, or automatic breaking migration | Digest-only image reference, exact trusted host/proxy entries, edge header stripping, verify-full PostgreSQL and verified Redis TLS, local-only Horizon gate, separate migrator role, expand/contract and previous-image rollback |
