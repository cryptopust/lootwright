...
| Path of Building Community PoE2 | PoE2 | Format `APPROVED_WITH_LIMITS`; game-data reference only | Format provenance is separate from Lootwright-authored canonical facts | Distinct PoB2 XML, pinned commit | Format interoperability | Parser contract; no embedded upstream rows |
...
The operator-controlled PoE1 passive tree remains default-off. PoE2 0.3.0
canonical records and its deterministic manifest are now approved as a local,
edition-isolated dataset; unsupported facts remain explicit and no PoE1 data is
used as fallback.
