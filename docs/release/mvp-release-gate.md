...
| --- | --- | --- |
| PoE1 | **FAIL** | Real PoE1 parsing, exact governed ruleset resolution, normalized build hydration, deterministic findings, checksum verification, and provenance persistence are implemented. The production engine nevertheless emits empty recommendation and manual Trade-recipe arrays, uses an unspecified intent, and therefore does not turn the player's goal, budget, or locked equipment into advice. |
...
| Upgrade planner in production journey | FAIL | FAIL | Domain planner and PoE1 candidate factory exist, but production analysis does not call them. Goals, budget, and locked equipment therefore do not affect persisted advice. |
| Manual Trade recipes in production journey | FAIL | FAIL | Edition-specific manual recipe architecture exists, but production output persists no recipes. Canonical modifier/Trade-vocabulary coverage is unavailable for the active production journey. |
...
| AI-off deterministic operation | PASS_WITH_LIMITATIONS | FAIL | PoE1 findings work with AI off. The useful planner/recipe portion of the requested workflow does not. |
...
chain stops after the finding because no production upgrade candidate is
persisted:

```text
user goal -> finding -> evidence -> rule -> [STOP]
```

To pass, production output must continue through candidate, constraint and
budget evaluation, market uncertainty/evidence, deterministic order, and a
manual recipe or explicit unsupported-vocabulary result. Every node must retain
the same edition and exact ruleset identity.
...
- `composer run test`: passed, 984 tests with 982 passed, 2 skipped, and 13,854
  assertions.
- `composer run test:acceptance`: passed, 6 tests and 60 assertions. These
  tests deliberately confirm the production planner/recipe gap and therefore
  do not imply a release PASS.
- `npm ci`: passed.
...
1. Map immutable analysis parameters into a typed PoE1 `BuildIntent`,
   `BudgetConstraint`, and `UserConstraints`; the current production engine
   deliberately uses `BuildIntent::unspecified`.
2. Invoke the deterministic `UpgradePlanner` in the production PoE1 workflow
   and persist its ordered candidate graph plus ordering reasons.
3. Persist a machine-readable trace joining goal, finding, evidence, rule,
   candidate, constraints, market evidence/uncertainty, and recommendation.
4. Import and approve sufficient edition-scoped modifier and Trade vocabulary,
   then generate manual recipes only for validated requirements. Unknown
   filters must remain unsupported.
5. Connect approved, timestamped market observations to budget evaluation; do
   not block structural advice or invent prices when observations are absent.
6. Record stage-specific import, analysis, planner, recipe, and optional AI
   latency instead of relying only on workflow wall-clock timestamps.
7. Execute and sign the real PoE1 staging checklist with representative builds
   and player questions, including locked equipment and AI red-team attempts.
8. Run `migrate:fresh`, rollback/reapply, and the full suite against PostgreSQL.
9. For PoE2, independently provide an approved canonical dataset/ruleset,
...
until blocker 9 is completed independently.
