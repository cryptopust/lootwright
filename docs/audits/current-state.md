...
| Rulesets | **PARTIAL** | Immutable source snapshots, quarantine/conflicts, published ruleset versions, source links, activation history, exact edition/patch/league/parser resolution, PostgreSQL immutability triggers, and atomic activation exist. No general-purpose PoE2 ruleset exists; activation remains an explicit operator workflow. |
